...
void set_value(struct Matrix* mat, double value, int row, int col) {
    int index = row * mat->cols + col;
    mat->bufs[index] = value;
};

void set_value_by_index(struct Matrix* mat, double value, int index){
    mat->bufs[index] = value;
}
...
void add_value(struct Matrix* mat, double value, int row, int col){
    double pre_value = get_value(mat, row, col);
    set_value(mat, pre_value + value, row, col);
}
...
struct Matrix* transepose_mat(const struct Matrix* original_mat) {
    ...
    for (i = 0; i < original_mat->rows; i++) {
        for (j = 0; j < original_mat->cols; j++) {
            double value = get_value(original_mat, i, j);
            set_value(transposed_mat, value, j, i);
        }
    }
